...
function normalizeMarkValue(value: string): string {
  return value.toLowerCase().replace(/ё/g, 'е').replace(/\s+/g, '').replace(/[.,]/g, '');
}
...
function getGradeClass(value: string | null | undefined): string {
  ...
  if (['н', 'нб', 'н/б', 'нб/у'].includes(normalized)) {
    return 'mark mark--absence';
  }

  if (['ну', 'н/у'].includes(normalized)) {
    return 'mark mark--valid-absence';
  }

  if (['эн', 'э/н', 'энка', 'энки'].includes(normalized)) {
    return 'mark mark--electronic-absence';
  }

  if (['зач', 'зачет', '+'].includes(normalized)) {
      ...
      {activeTab === 'report-cards' ? (
                      ...
                      <tbody>
                        {card.rows.map((row) => (
                          <tr key={`${card.studentId}-${row.index}-${row.subjectName}`}>
                            ...
                            <td className="center-cell"><MarkBadge value={row.average} /></td>
                            <td className="center-cell"><AbsenceBadge value={row.absences.valid} type="valid" /></td>
                            <td className="center-cell"><AbsenceBadge value={row.absences.invalid} type="invalid" /></td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
        ...
        <section className="subject-section">
          <div className="subject-header">
            ...
            <div className="mark-legend" aria-label="Цветовые обозначения">
              <span><MarkBadge value="5" /> отлично</span>
              <span><MarkBadge value="4" /> хорошо</span>
              <span><MarkBadge value="3" /> удовлетворительно</span>
              <span><MarkBadge value="2" /> риск</span>
              <span><MarkBadge value="Н" /> пропуск</span>
              <span><MarkBadge value="ЭН" /> энка</span>
            </div>
          </div>
