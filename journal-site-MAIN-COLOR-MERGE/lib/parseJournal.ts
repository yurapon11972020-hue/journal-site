...
function parseReportCards(workbook: XLSX.WorkBook, students: StudentRecord[]): ReportCard[] {
  ...
  for (let index = 0; index < blockStarts.length; index += 1) {
    ...
    for (let row = startRow + 3; row <= subjectsEndRow; row += 1) {
      ...
      const sessionLabel = getCellText(sheet, row, 3) || null;
      const averageLabel = getCellText(sheet, row, 4) || null;
      const validAbsenceLabel = getCellText(sheet, row, 5) || null;
      const invalidAbsenceLabel = getCellText(sheet, row, 6) || null;
